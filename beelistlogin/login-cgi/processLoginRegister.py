...
"""
this script use to user login and register process
"""
print "Content-Type: text/html\n\n";
print "<!DOCTYPE html>";
import sys, json, urllib, cgi, os, dbmodule_pg, cgitb, psycopg2,random,requests,time
...
r = requests.get('http://missouri.agriculture.purdue.edu/cgi-bin/login/activate.cgi/get', params=payload)
...
if "register-submit" in formData:
    try:
        ...
        if exist == None:
            ...
            print '<script language="javascript">';
            print 'alert("email already registered.Now redirecting to login page.")';
            print 'window.location = "http://missouri.agriculture.purdue.edu/register.html"';
            print '</script>';
...
if "login-submit" in formData:
    try:
        con = dbmodule_pg.getconn()
        cur = con.cursor()
        loginsql = "SELECT email FROM lthia.users WHERE email ='%s' AND password ='%s';" % (email,password)
        ...
        if exist == None:
            ...
            print "Set-Cookie:Expires=time.time() + 15 * 24 * 3600;\r\n"
            #print "Set-Cookie:Domain=www.missouri.agriculture.purdue.edu;\r\n"