...
if "login-submit" in formData:
    try:
        ...
        if exist == None:
            ...
            estr = ''.join(str(timestamps))
            
            #fetch all table session_id that created by this user.
            sessionsql = "SELECT session_id FROM lthia.results WHERE login_p ='%s' AND bmpclass = 'Initial' ;" % (email)
            cur.execute(sessionsql)
            con.commit()
            rowss = cur.fetchall()
            sessions=[]
            for serow in rowss:
                sessions.append(serow)
            sessionstr = ''.join(str(sessions))
            print '<script language="javascript">';
            print 'sessionStorage.setItem("sessions","'+sessionstr+ '");'
            print 'sessionStorage.setItem("email","'+email+ '");'
            print 'sessionStorage.setItem("timestamps","'+estr+ '");'
